...
The test subject makes an average of `r median(stepsperday$steps, na.rm = T)`
steps per day.
...
```
Hence, at the `r indexmaxinterval`th's interval, corresponding to
`r stepsperinterval$interval[indexmaxinterval]`, at 8:35 am, most steps are
taken on average.
...
higher than before
(`r round(median(stepsperday2$steps) - median(stepsperday$steps, na.rm=T), 2)` steps)
